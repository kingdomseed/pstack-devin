...
i want two devin automations that work together in one slack issue channel.
...
- i want pstack available to devin sessions in the target repository through a committed `.devin/config.json` `requiredPlugins` entry only, for shared dependencies such as `how`, `why`, `tdd`, `unslop`, and the required principle skills.
...
- session mode: `<devin mode for triage and repro sessions>`
...
start from [`configuration.example.yaml`](./templates/configuration.example.yaml) and [`feature-map.example.md`](./skills/reproduce-and-fix-issues/references/feature-map.example.md). copy and fill them outside this pack, for example under `.devin/benny/`. keep secret values in devin org secrets or a secret manager, and reference them in prompts as `${SECRET_NAME}` tokens.
...
i want you to merge this entry into the target repository's `.devin/config.json`:
...
use a source form that resolves in cloud sessions: a pushed github repo (`owner/repo` or `owner/repo#plugins/pstack`), a git url, or a `git-subdir` object. a `--local` install or a local path does not reach devin cloud automation sessions. preserve every unrelated setting. preserve comments and valid jsonc syntax when the existing file uses jsonc.

i want verification from a fresh devin session rooted in the target repository. confirm that pstack's `how`, `why`, `tdd`, `unslop`, and the principle skills used by benny resolve as `/pstack:` skills in project scope. do not count skills loaded from the current session or a user-scoped install.

if repo-scoped plugin requirements are unavailable or any shared dependency does not resolve, stop and explain what failed. do not add `.devin/automations/benny/skills/` to a plugin manifest or expect its files to appear in the slash-skill list.
...
for first-time creation, build each automation at app.devin.ai under Settings, Automations, or through the v3 api (`POST /v3/organizations/{org}/automations`). finish and review the first automation before starting the second. the trigger is `slack:message` with a condition matching the configured source channel; add the `attach_thread` reply binding so the session stays bound to the triggering thread. the action is `start_session` with a prompt that paraphrases this intent and the finished configuration.

the triage prompt must read and follow `.devin/automations/benny/skills/triage-issue-reports/SKILL.md`. the repro prompt must read and follow `.devin/automations/benny/skills/reproduce-and-fix-issues/SKILL.md`. use these repo-relative paths only after you confirm they are committed in the repository where the automation's sessions will run, and reference the repo with an `@{owner}/{repo}` prompt token so the session clones it.

for existing automations, validate the configuration, then use the concise field checklist in the copied setup file so i can edit each automation directly in its editor or through `PATCH /v3/organizations/{org}/automations/{automation_id}`. do not create duplicates.

PORT-NOTE: devin has no `/automate` draft-review flow; creation is a direct save in the webapp editor or a v3 api call, so the review discipline lives in this file's checklists instead.
