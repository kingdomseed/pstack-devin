...
- **Treating `auto` as a profile name.** `auto` and `inherit-parent` mean `subagent_general`: the role runs on the parent chat model. And a custom profile with no `model:` field does not inherit the parent; it falls back to the router default. [Setup](./01-setup.md) covers the roles.
