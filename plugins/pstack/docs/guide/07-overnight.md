...
A local session dies with your laptop. For a run that must outlive it, hand it off instead: `/handoff` moves the session to a cloud Devin on its own VM, and it keeps going after you close the lid. Plugin subagent profiles don't load in cloud sessions, so an overnight run that fans out does so on the built-in profiles there. [Running pstack on Devin](./00-devin.md) has the full local-versus-cloud picture.
