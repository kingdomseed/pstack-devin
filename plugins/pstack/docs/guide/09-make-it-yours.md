...
You don't describe your style, because [`/pstack:automate-me`](../../skills/automate-me/SKILL.md) reads it out of your history. It mines your recent transcripts under `~/.local/share/devin/cli/transcripts/` for repeated preferences, in how you like replies, delegation, verification, code, prose, and process, then asks you which patterns are really you. It drafts `.devin/skills/<your-name>-mode/SKILL.md` in Devin's skill format, runs the draft through [`/pstack:unslop`](../../skills/unslop/SKILL.md), and opens a PR from a worktree so you review it like any other change.
...
Writing a skill matches the [Authoring or modifying a skill playbook](../../skills/poteto-mode/playbooks/authoring-a-skill.md), which writes the `SKILL.md` directly in Devin's format (frontmatter plus prompt body under `.devin/skills/<name>/`), validates the frontmatter and links, and ships the result through the Opening a PR playbook. Agent-facing prose has a higher bar than human prose, because an unhelpful sentence becomes an instruction some future agent follows. Let the playbook hold that bar rather than writing a `SKILL.md` freehand.
