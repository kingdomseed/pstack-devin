...
Build a real mental model of every system the new code touches. Run the **how** skill (`/pstack:how`) over the relevant subsystems.

Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design redefines ownership or layering, also run the **why** skill (`/pstack:why`) on the existing shape so the rationale becomes a constraint, not a guess.
...
Use your configured `architect runners` from `~/.devin/rules/pstack-models.md` (defaults `pstack:sol`, `pstack:terra`, `pstack:kimi`, `subagent_general`).
...
1. Re-run the **how** skill (`/pstack:how`) over what's been built.
