...
- `read` calls against any `SKILL.md` file (project `.devin/skills/`, user-level `~/.config/devin/skills/`, or plugin-installed paths under `~/.local/share/devin/cli/plugins/`)
- `run_subagent` prompts that name a skill path
- Tool calls (`exec`, `grep`, MCP, etc.) that match a skill's documented commands
