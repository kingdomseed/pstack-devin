...
1. Spawn `run_subagent` with profile `pstack:comment-sicko`. Pass the scope. Do not restate its rules. The profile loads in local sessions only; in a cloud session run the same review inline yourself.
