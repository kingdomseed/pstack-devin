...
Transcripts live at `~/.local/share/devin/cli/transcripts/<session-id>.json`, one JSON file per session with `session_id` at the top. Cheap digests live at `~/.local/share/devin/cli/summaries/history_*.md`; scan digests to triage, then open only the full transcripts that matter. Each session's `system_info` message records its workspace directories, which is how you scope a transcript to this project.
...
5. Verify against live state. Take the PRs, branches, and tickets that the mining and the sweep surfaced and check them with `git` and `gh`. When the answer hinges on what an agent actually did (the tools it ran, files it read, errors it hit), read the full transcript JSON, not just its summary digest.
