...
Investigation fans out `/pstack:how` + `/pstack:why` as parallel subagents. Plugin `pstack:*` profiles load in local sessions only; a cloud session degrades to the built-in `subagent_explore`/`subagent_general` profiles.
