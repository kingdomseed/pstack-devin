...
3. Plan the fix from the trace. If it crosses a function boundary, `/pstack:architect` first. Delegate implementation to a subagent on your configured `perf-issue` profile from `~/.devin/rules/pstack-models.md` (default `pstack:sol`). Review the diff. Capture a post-fix trace.
