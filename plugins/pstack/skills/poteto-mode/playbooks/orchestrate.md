...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through `run_subagent` (local) or the Devin session API (cloud). State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3 via `max-nesting` on the profile; nested spawns are local and cannot start cloud sessions). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always a Devin cloud session unless the task needs this machine: runtime verification on a local surface (a terminal-run CLI or TUI, `browser_preview`, or a GUI driver). Reading local transcripts under `~/.local/share/devin/cli/transcripts/` and `~/.local/share/devin/cli/summaries/`. Simulators and local IDE state. Auth that exists only here. Cloud sessions cannot read the local store, so their briefs inline what they need or point at repo paths. A cloud session's own fan-out sees this plugin's skills and rules but not its `pstack:*` profiles, so it degrades to `subagent_general` and `subagent_explore`. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via `/loop`, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud session's status in the Devin web app (app.devin.ai). Transcript mtime is not liveness.
...
- After a Devin restart: local subagents park and reattach on session resume, and cloud sessions keep running regardless. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach cloud work by PR and branch rather than session id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
...
PORT-NOTE: `run_subagent` cannot create Devin cloud sessions; spawn workers through `/handoff`, the Devin session API, or app.devin.ai, and treat their completion as queue events observed through pushed branches and PRs rather than subagent notifications.
PORT-NOTE: Upstream read a store path from the system prompt. Devin exposes no per-agent store path, so the store location is a convention (`orchestrate/<project-slug>/` under a gitignored dir or `~/.local/share/devin/`).
